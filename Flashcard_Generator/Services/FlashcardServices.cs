...
namespace Flashcard_Generator
{
	public class FlashcardServices
	{
		...
		#region QUERY FUNCTIONS - SPECIFIC GETTERS
		...
		public List<Flashcard> GetFlashcardsByLanguagesCategoriesAndVisibility(string source, string target, string category, string loggedUser, bool isOwner)
		{
			...
			using (SqlConnection con = new SqlConnection(connectionString))
			{
				...
				if (loggedUser == null)
				{
					query = @"SELECT f.id_flashcard, u.username, f.source_language, 
					f.target_language, f.category, f.word_source, f.word_target, 
					f.example_sentence_source, f.example_sentence_target, 
					f.pronunciation,f.tips,f.proficiency, f.is_public	
					FROM FLASHCARDS f JOIN USERS u ON f.id_user = u.id_user 
					WHERE f.source_language = @source 
					AND f.target_language = @target 
					AND f.category = @category 
					AND f.is_public = 1
					ORDER BY f.proficiency, f.id_flashcard, f.word_target";
				}
				else
				{
					if (!isOwner)
					{
						query = @"SELECT f.id_flashcard, u.username, f.source_language, f.target_language, 
						f.category, f.word_source, f.word_target, f.example_sentence_source, f.example_sentence_target, 
						f.pronunciation,f.tips,f.proficiency,f.is_public	
						FROM FLASHCARDS f JOIN USERS u ON f.id_user = u.id_user 
						WHERE f.source_language = @source 
						AND f.target_language = @target 
					...
					{
						query = @"SELECT f.id_flashcard, u.username, f.source_language, f.target_language, f.category, 
						f.word_source, f.word_target, f.example_sentence_source, f.example_sentence_target, f.pronunciation,
						f.tips,f.proficiency,f.is_public	
						FROM FLASHCARDS f JOIN USERS u ON f.id_user = u.id_user 
						WHERE f.source_language = @source 
						AND f.target_language = @target 
						AND f.category = @category 
		...
		public MemoryStream GeneratePDF(List<Flashcard> flashcards, string fontPath)
		{
			...
			Font headerFont = new Font(bf, 6, Font.NORMAL, BaseColor.BLACK);
			Font titleFont = new Font(bf, 10, Font.NORMAL, BaseColor.BLACK);
			Font textFont = new Font(bf, 12, Font.NORMAL, BaseColor.BLACK);
			Font smallFont = new Font(bf, 8, Font.NORMAL, BaseColor.BLACK);
				...
				for (int i = 0; i < pages; i++)
				{
					...
					for (int j = fromThisFlashcard; j < tillThisFlashcard; j++)
					{
						PdfPCell cell = new PdfPCell(CreateFlashcardTable(headerFont, titleFont, textFont, smallFont,
							flashcards[j].TargetLanguage, flashcards[j].Category, flashcards[j].WordTarget, flashcards[j].ExampleSentenceTarget,
					...
					for (int j = fromThisFlashcard; j < tillThisFlashcard; j += 2)
					{
						...
						{
							PdfPCell cell = new PdfPCell(CreateFlashcardTable(headerFont, titleFont, textFont, smallFont,
								flashcards[j + 1].SourceLanguage, flashcards[j].Category, flashcards[j + 1].WordSource,
						...
						if (j < numberOfFlashcards)
						{
							PdfPCell cell2 = new PdfPCell(CreateFlashcardTable(headerFont, titleFont, textFont, smallFont,
								flashcards[j].SourceLanguage, flashcards[j].Category, flashcards[j].WordSource,
		...
		private PdfPTable CreateFlashcardTable(Font headerFont, Font titleFont, Font textFont, Font smallFont, string language, string category,
	string word, string sentence, string additionalInfo, string level, int id)
		{
			PdfPTable table = new PdfPTable(1);
			table.WidthPercentage = 100;
			table.SpacingAfter = 0; // Ensure no extra space after the table
			...
			cardContainer.PaddingBottom = 0; // Ensure no bottom padding
			cardContainer.Border = PdfPCell.BOX;
			cardContainer.BorderColor = BaseColor.BLACK;
			cardContainer.BorderWidth = 2f;

			// Card header
			PdfPTable headerTable = new PdfPTable(3);
			headerTable.WidthPercentage = 100;
			headerTable.SetWidths(new float[] { 1, 4, 1 });
			headerTable.SpacingAfter = 0; // No extra space after header table
			...
			headerTable.AddCell(new PdfPCell(new Phrase(language + " - " + category, headerFont)) { Border = PdfPCell.NO_BORDER, HorizontalAlignment = Element.ALIGN_CENTER });
			...
			Paragraph wordParagraph = new Paragraph(word, titleFont)
			{
				Alignment = Element.ALIGN_CENTER,
				SpacingAfter = 3f // Adjust this value for spacing after
			};

			Paragraph sentenceParagraph = new Paragraph(sentence, textFont)
			{
				Alignment = Element.ALIGN_CENTER,
				SpacingAfter = 3f // Adjust this value for spacing after
			};

			Paragraph additionalInfoParagraph = new Paragraph(additionalInfo, smallFont)
			{
				Alignment = Element.ALIGN_CENTER
			};
